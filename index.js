...
{
  "id"; 8,
  "name"; "Vincent van Gough", //console.log Vincent Van Gough
  "years"; "1853 - 1890",
...
{"id"; 20,
  ...
  "Bio"; "Greg Olsen Born in Idaho Falls Idaho is a prominent LDS artist that paints pictures of the Savior Jesus Christ"
  "paintings"; 300
}
...
// 'if'(getArtistByIndex) is  `invoked` `with` (`artists`) `Array`(1), it `will` `return` `the artist at index 88 is Vasiliy Kandinskiy` *
...
//Invoking addArtists( artists, `Vincent Van Gough`, 30 March 1853 – 29 July 1890) should return the artists Array(8) with the bio added to the array

//`addArtist` `Greg Olsen`, `1958-present, "Religious Paintings", paintings (300)`` 
//"Nationality - Caucasion"
//``Bio; reg Olsen Born in Idaho Falls Idaho is a prominent LDS artist that paints pictures of the Savior Jesus Christ.)`
...
function lotsOfArt(artists) {will `return` [ `El Greco` , `Albrecht Dürer`, `Peter Paul Rubens`]
...
{
`Invoking` `artistInfo` (`Vincent van Gough`) `will` `return`;
...
//int-disable-next-line no-unused-vars:
//lastline 